...
void lchan_calc_crc(LogicalChannel chan) {
  ...
  if (chan->crc_type * 8 == CRC16) {
    uint16_t crc = crc_generate_key(LIQUID_CRC_16, chan->data,
                                    chan->payload_len - chan->crc_type);
    chan->data[chan->payload_len - 2] = (crc >> 8) & 0xFF; // upper byte
    chan->data[chan->payload_len - 1] = crc & 0xFF;        // lower byte
  } else {
    uint16_t crc = crc_generate_key(LIQUID_CRC_8, chan->data,
                                    chan->payload_len - chan->crc_type);
...
int lchan_verify_crc(LogicalChannel chan) {
  if (chan->crc_type * 8 == CRC16) {
    ...
    return crc_validate_message(LIQUID_CRC_16, chan->data,
                                chan->payload_len - chan->crc_type, crc);
  } else {
    uint16_t crc = chan->data[chan->payload_len - 1];
    return crc_validate_message(LIQUID_CRC_8, chan->data,
                                chan->payload_len - chan->crc_type, crc);
  }
}
