...
int mac_ue_handle_message(MacUE mac, MacMessage msg, uint is_broadcast) {
  ...
  case associate_response:
    ...
    if (mac->is_associated ||
        (msg->hdr.AssociateResponse.rachuserid != mac->phy->rachuserid))
      break;
    ...
    if (msg->hdr.AssociateResponse.response == assoc_resp_success) {
      ...
      LOG_SFN_MAC(INFO, "[MAC UE] successfully associated! userid: %d\n",
                  mac->userid);
      mac->phy->userid = mac->userid; // Notify phy about the userid. TODO
...
void mac_ue_run_scheduler(MacUE mac) {
  ...
  int num_assigned = num_slot_assigned(mac->ul_data_assignments,
                                       MAC_ULDATA_SLOTS, UE_ASSIGNED);
  uint next_sfn = (mac->phy->common->tx_subframe + 1) %
                  2; // subframe for which the scheduler is run
  // Log schedule
  LOG(TRACE, "[MAC UE] Scheduler user assignments:\n");
  LOG(TRACE, "         DL data slots: %4d %4d %4d %4d\n",
      mac->dl_data_assignments[0], mac->dl_data_assignments[1],
      mac->dl_data_assignments[2], mac->dl_data_assignments[3]);
  LOG(TRACE, "         UL data slots: %4d %4d %4d %4d\n",
      mac->ul_data_assignments[0], mac->ul_data_assignments[1],
      mac->ul_data_assignments[2], mac->ul_data_assignments[3]);
  LOG(TRACE, "         UL ctrl slots: %4d %4d\n", mac->ul_ctrl_assignments[0],
      mac->ul_ctrl_assignments[1]);
      ...
      if (mac->ul_data_assignments[i] == 1) {
        ...
        if (queuesize > 0) {
          ...
          if (num_assigned == 0 && queuesize > lchan_unused_bytes(chan)) {
            ...
            uint remaining_bytes = queuesize - lchan_unused_bytes(chan) +
                                   mac_msg_get_hdrlen(ul_req);
  ...
  if (num_slot_assigned(mac->ul_ctrl_assignments, MAC_ULCTRL_SLOTS, 1) > 0) {
    ...
    LogicalChannel chan =
        lchan_create(get_ulctrl_slot_size(mac->phy->common) / 8, CRC8);
    lchan_add_all_msgs(chan, mac->msg_control_queue);
    lchan_calc_crc(chan);
...
int mac_ue_is_associated(MacUE mac) { return mac->is_associated; }
...
int mac_ue_get_timing_advance(MacUE mac) { return mac->timing_advance; }
