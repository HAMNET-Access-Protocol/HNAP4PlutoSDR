...
// if crit_delay_us is set greater than zero, it is checked if the current
// iteration took longer than the allowed crit_delay
void timecheck_stop(struct timecheck_s *time, int crit_delay_us) {
  struct timespec end;
  clock_gettime(CLOCK_MONOTONIC, &end);
  float elapsed = (end.tv_sec - time->start.tv_sec) * 1000000.0 +
                  (end.tv_nsec - time->start.tv_nsec) / 1000.0;
  time->avg += elapsed;
  time->count++;
  if (elapsed > time->max)
    time->max = elapsed;
...
void timecheck_info(struct timecheck_s *time) {
  if (time->count % time->avg_len == 0) {
    LOG(INFO, "[TimingMonitor] '%10s': avg:%4.1fus max:%4.1f after %5d iter\n",
        time->name, time->avg / time->avg_len, time->max, time->count);
    time->avg = 0;
    time->max = 0;
  }
}
