...
int _ue_rx_symbol_cb(float complex *syms, uint M, void *userd);
...
PhyUE phy_ue_init() {
  ...
  phy->rec = sc_receiver_create(SC_SYMBOL_UNIT, _ue_rx_symbol_cb, phy);
...
void phy_ue_set_mac_interface(PhyUE phy,
                              void (*mac_rx_cb)(struct MacUE_s *,
                                                LogicalChannel, uint),
                              struct MacUE_s *mac) {
...
// returns -1 if no sync found, else the sample index of the ofdm symbol after
// the sync sequence
int phy_ue_initial_sync(PhyUE phy, float complex *rxbuf_time,
                        uint num_samples) {
  PhyCommon common = phy->common;

  int offset = sc_receiver_get_sync(phy->rec, rxbuf_time, num_samples);
  if (offset != -1) {
    common->rx_symbol =
        SUBFRAME_LEN - 1 - 1; // there is the sync info symbol and one guard
                              // symbol remaining in the subframe
    ...
    if (phy->has_synced_once == 0) {
      ...
      common->tx_active = 1;
      common->tx_symbol =
          common->rx_symbol + 1 - DL_UL_SHIFT +
          DL_UL_SHIFT_COMP_UE; // TODO clarify what happens for offset=0
      common->tx_subframe = 0;

      phy->has_synced_once = 1;
      LOG(INFO, "[PHY UE] Got sync!  %d samps\n", offset);
      SYSLOG(LOG_INFO, "[PHY UE] Got sync! offset: %d samps\n", offset);
    } else {
      LOG_SFN_PHY(DEBUG, "[PHY UE] sync seq. offset: %d samps\n", offset);
    }
  }
  return offset;
}
...
int phy_ue_proc_dlctrl(PhyUE phy) {
  ...
  // demodulate signal.
  uint llr_len = 2 * DLCTRL_LEN * SC_SYMBOL_UNIT;
  uint8_t *llr_buf = malloc(llr_len);
  uint total_samps = 0;
  phy_demod_soft(common, 0, DLCTRL_LEN * SC_SYMBOL_UNIT - 1, 0, llr_buf,
                 llr_len, &total_samps);

  // soft decoding
  dlctrl_alloc_t *dlctrl_buf = malloc(dlctrl_size + 1);
  fec_decode_soft(common->fec_ctrl, dlctrl_size + 1, llr_buf,
                  (uint8_t *)dlctrl_buf);

  // unscrambling
  unscramble_data((uint8_t *)dlctrl_buf, dlctrl_size + 1);
  // verify CRC
  if (!crc_validate_message(LIQUID_CRC_8, (uint8_t *)dlctrl_buf, dlctrl_size,
                            dlctrl_buf[dlctrl_size].byte)) {
  ...
  for (int i = 0; i < NUM_SLOT / 2; i++) {
    ...
    phy->dlslot_assignments[sfn][2 * i] =
        dlctrl_buf[idx].h4 == USER_BROADCAST
            ? BRCST_ASSIGNED
            : phy->dlslot_assignments[sfn][2 * i];
    phy->dlslot_assignments[sfn][2 * i + 1] =
        dlctrl_buf[idx].l4 == phy->userid ? UE_ASSIGNED : NOT_ASSIGNED;
    phy->dlslot_assignments[sfn][2 * i + 1] =
        dlctrl_buf[idx].l4 == USER_BROADCAST
            ? BRCST_ASSIGNED
            : phy->dlslot_assignments[sfn][2 * i + 1];
    idx++;
  }
...
void phy_ue_proc_slot(PhyUE phy, uint slotnr) {
  ...
  PhyCommon common = phy->common;
  assignment_t slot_type =
      phy->dlslot_assignments[common->rx_subframe % 2][slotnr];
  if (slot_type != NOT_ASSIGNED) {
    ...
    uint first_symb =
        SC_SYMBOL_UNIT * (DLCTRL_LEN + 2 + (SLOT_LEN + 1) * slotnr);
    uint last_symb = first_symb + SC_SYMBOL_UNIT * SLOT_LEN - 1;
    TIMECHECK_START(check_demod);
    phy_demod_soft(common, first_symb + SC_SYMBOL_UNIT, last_symb, mcs,
    ...
    // we start calculating ber after subframe 50 to wait that MCS switch
    // happened
    if (global_sfn > 50) {
      for (int i = 0; i < chan->payload_len; i++)
        num_biterr += liquid_count_ones(
            phy_dl[common->rx_subframe % 2][slotnr][i] ^ chan->data[i]);
...
int phy_ue_proc_sync_info(PhyUE phy) {
  ...
  // demodulate signal
  const int symb_idx = SC_SYMBOL_UNIT * (SUBFRAME_LEN - 2);
  uint written_samps = 0;
  phy_demod_soft(common, symb_idx, symb_idx + SC_SYMBOL_UNIT - 1, 0, demod_buf,
...
int _ue_rx_symbol_cb(float complex *X, uint M, void *userd) {
  PhyUE phy = (PhyUE)userd;
  PhyCommon common = phy->common;

  if (common->rx_symbol == 4 || common->rx_symbol == 4 + SLOT_LEN + 1 ||
      common->rx_symbol == 4 + 2 * (SLOT_LEN + 1) ||
      common->rx_symbol == 4 + 3 * (SLOT_LEN + 1)) {
    sc_receiver_eq_update(phy->rec, X, SC_SYMBOL_UNIT);
  }
  sc_receiver_equalize(phy->rec, X, SC_SYMBOL_UNIT);

  memcpy(&common->rxdata[SC_SYMBOL_UNIT * common->rx_symbol], X,
  ...
  if ((common->rx_subframe == 0) &&
    ...
    // TODO set new gain value
    // phy->rssi = (1-agc_rssi_filt_param)*phy->rssi +
    // agc_rssi_filt_param*ofdmframesync_get_rssi(phy->fs); store old cfo
    // estimation
    sc_receiver_reset(phy->rec);
  }

  // Debug log
  /*char name[30];
  sprintf(name,"rxF/ue_rxsym_%d_%d.m",common->rx_subframe,common->rx_symbol-1);
    ...
    if (!mac_ue_is_associated(phy->mac)) {
      ...
      if (common->tx_subframe == 0 && tx_symb == SUBFRAME_LEN - SLOT_LEN - 1) {
        ...
        phy->platform->ptt_set_tx(phy->platform);
                 ...
                 tx_symb == SUBFRAME_LEN - SLOT_LEN) {
        phy_pulse_shaper(common, txbuf_time, zadoff_chu_seq, SC_SYMBOL_UNIT);
      } else if (common->tx_subframe == 0 &&
                 tx_symb == SUBFRAME_LEN - SLOT_LEN + 1) {
        phy_pulse_shaper(common, txbuf_time, &zadoff_chu_seq[SC_SYMBOL_UNIT],
                         SC_SYMBOL_UNIT);
      } else if (common->tx_subframe == 0 &&
                 tx_symb == SUBFRAME_LEN - SLOT_LEN + 2) {
        phy_pulse_shaper(common, txbuf_time,
                         &zadoff_chu_seq[2 * SC_SYMBOL_UNIT], SC_SYMBOL_UNIT);
      } else if (common->tx_subframe == 0 &&
                 tx_symb == SUBFRAME_LEN - SLOT_LEN + 3) {
        phy_ue_create_assoc_request(phy, txbuf_time);
      } else if (common->tx_subframe == 0 &&
                 tx_symb == SUBFRAME_LEN - SLOT_LEN + 4) {
        phy->platform->ptt_set_rx(phy->platform);
      } else {
        // send zeros
        phy_pulse_shaper_zeros(common, txbuf_time, SC_SYMBOL_UNIT);
      }
...
// receive an arbitrary number of samples and process slots once they are
// received
void phy_ue_do_rx(PhyUE phy, float complex *rxbuf_time, uint num_samples) {
  ...
  while (remaining_samps > 0) {
    // find sync sequence
    if (phy->rec->sync_state == SEARCH_ZADOFF) {
      ...
      // receive symbols
      uint rx_sym = fmin(SC_SYMBOL_UNIT * OSF, remaining_samps);
      sc_receiver_execute(phy->rec, rxbuf_time, rx_sym);
...
int phy_map_ulctrl(PhyUE phy, LogicalChannel chan, uint subframe,
                   uint8_t slot_nr) {
  PhyCommon common = phy->common;
  ...
  uint total_samps = 0;
  uint first_symb =
      2 * (SLOT_LEN + 1) + 2 * slot_nr; // slot 0 is mapped to symbol 30, slot 1
                                        // is mapped to symb 32.

  // modulate signal
  uint sfn = subframe % 2;
  phy_mod(phy->common, sfn, first_symb * SC_SYMBOL_UNIT,
          (first_symb + 1) * SC_SYMBOL_UNIT - 1, mcs, repacked_b, num_repacked,
          &total_samps);

  // activate used OFDM symbols in resource allocation
  if (phy->ul_symbol_alloc[sfn][first_symb - 2] == NOT_USED)
    phy->ul_symbol_alloc[sfn][first_symb - 1] =
        PTT_UP; // indicate PTT, slot before isnt used
  phy->ul_symbol_alloc[sfn][first_symb] = DATA;
  if (phy->ul_symbol_alloc[sfn][first_symb + 2] == NOT_USED)
    phy->ul_symbol_alloc[sfn][first_symb + 1] =
        PTT_DOWN; // next slot is not used, end PTT here
...
int phy_map_ulslot(PhyUE phy, LogicalChannel chan, uint subframe,
                   uint8_t slot_nr, uint mcs) {
  PhyCommon common = phy->common;
  ...
  uint last_symb =
      (SLOT_LEN + 1) * (slot_nr + 1) -
      2; // TODO implement generic function to calc all slot allocations
  ...
  memcpy(&common->txdata[sfn][first_symb * SC_SYMBOL_UNIT], training_seq,
         SC_SYMBOL_UNIT * sizeof(float complex));
  // modulate signal
  phy_mod(phy->common, sfn, (first_symb + 1) * SC_SYMBOL_UNIT,
          (last_symb + 1) * SC_SYMBOL_UNIT - 1, mcs, repacked_b, num_repacked,
          &total_samps);

  // activate used OFDM symbols in resource allocation
  memset(&phy->ul_symbol_alloc[sfn][first_symb], DATA,
         last_symb - first_symb + 1);
  // if the previous slot is not used, we have to set the PTT signal before this
  // data slot
  if (slot_nr == 0) {
    if (phy->ul_symbol_alloc[(sfn - 1) % 2][SUBFRAME_LEN - 2] == NOT_USED)
      phy->ul_symbol_alloc[(sfn - 1) % 2][SUBFRAME_LEN - 1] =
          PTT_UP; // indicate PTT, slot before isnt used
    else
      phy->ul_symbol_alloc[(sfn - 1) % 2][SUBFRAME_LEN - 1] =
          DATA; // previous slot already in use. concat them
  } else {
    if (phy->ul_symbol_alloc[sfn][first_symb - 2] == NOT_USED)
      phy->ul_symbol_alloc[sfn][first_symb - 1] =
          PTT_UP; // indicate PTT, slot before isnt used
  ...
  if (slot_nr == NUM_SLOT - 1) {
    // for the last slot within a subframe we simply assume that the next slot
    // is unused. if it is used, the property can be overwritten in the next
    // subframe assignment
    phy->ul_symbol_alloc[sfn][last_symb + 1] =
        PTT_DOWN; // next slot is not used, end PTT here
  } else {
