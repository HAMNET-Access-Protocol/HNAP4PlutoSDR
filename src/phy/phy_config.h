...
#define PHY_TEST_BER
#else
// If this is set, a seperate thread for Viterbi Decoding and MAC RX is used.
// Should be activated for better performance If not used, the phy RX thread has
// to do Decoding and MAC RX itself.
...
// UE constantly adapts the rxgain based on the rssi. The rssi is calculated
// based on the sync signal, every 8 subframes. The new rssi value is smoothed
// with an exponential filter the filter coefficient alpha can be tuned here
...
int log_cfo_flag; // set this flag to enable logging the fine cfo estimates to a
                  // file
char cfo_logfile[80]; // name of the  fine cfo logfile

// ---------------------------- PHY config functions
// --------------------------------- // get the configuration from the specified
// file path
