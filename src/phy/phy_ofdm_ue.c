...
PhyUE phy_ue_init() {
  ...
  phy->fs = ofdmframesync_create(nfft, cp_len, 0, phy->common->pilot_sc,
                                 _ue_rx_symbol_cb, phy);
...
int phy_ue_proc_dlctrl(PhyUE phy) {
  ...
  uint total_samps = 0;
  phy_demod_soft(common, 0, nfft - 1, 0, DLCTRL_LEN - 1, 0, llr_buf, llr_len,
                 &total_samps);
...
void phy_ue_proc_slot(PhyUE phy, uint slotnr) {
  ...
  if (slot_type != NOT_ASSIGNED) {
    ...
    phy_demod_soft(common, 0, nfft - 1, first_symb, last_symb, mcs, demod_buf,
...
int phy_ue_proc_sync_info(PhyUE phy) {
  ...
  for (int i = 0; i < nfft; i++) {
    if (common->pilot_sc[i] == OFDMFRAME_SCTYPE_DATA &&
        written_samps < buf_len) {
      modem_demodulate_soft(common->mcs_modem[mcs],
                            common->rxdata_f[symb_idx][i], &symbol,
...
int _ue_rx_symbol_cb(float complex *X, unsigned char *p, uint M, void *userd) {
  ...
  memcpy(common->rxdata_f[common->rx_symbol++], X,
         sizeof(float complex) * nfft);
  ...
  if ((common->rx_subframe == 0) &&
    ...
    phy->rssi = (1 - agc_rssi_filt_param) * phy->rssi +
                agc_rssi_filt_param * ofdmframesync_get_rssi(phy->fs);
...
void phy_ue_create_assoc_request(PhyUE phy, float complex *txbuf_time) {
  ...
  for (int i = 0; i < nfft; i++) {
    if (common->pilot_sc[i] == OFDMFRAME_SCTYPE_DATA &&
        written_samps < num_repacked) {
      modem_modulate(common->mcs_modem[mcs], (uint)repacked_b[written_samps++],
                     &subcarriers[i]);
    }
  }
...
int phy_map_ulctrl(PhyUE phy, LogicalChannel chan, uint subframe,
  ...
  // modulate signal
  uint sfn = subframe % 2;
  phy_mod(phy->common, sfn, 0, nfft - 1, first_symb, first_symb, mcs,
...
int phy_map_ulslot(PhyUE phy, LogicalChannel chan, uint subframe,
  ...
  // modulate signal
  uint sfn = subframe % 2;
  phy_mod(phy->common, sfn, 0, nfft - 1, first_symb, last_symb, mcs, repacked_b,
