...
int phy_map_dlslot(PhyBS phy, LogicalChannel chan, uint subframe,
  ...
  // modulate signal
  phy_mod(phy->common, subframe, 0, nfft - 1, first_symb, last_symb, mcs,
...
void phy_map_dlctrl(PhyBS phy, uint subframe) {
  ...
  uint total_samps = 0;
  phy_mod(common, subframe, 0, nfft - 1, 0, DLCTRL_LEN - 1, mcs, repacked_b,
...
void phy_bs_proc_slot(PhyBS phy, uint slotnr) {
  ...
  uint last_symb = (SLOT_LEN + 1) * (slotnr + 1) -
                   2; // TODO use more constants and explain how to calc this
...
void phy_bs_proc_ulctrl(PhyBS phy, uint slotnr) {
  ...
  uint last_symb =
      (SLOT_LEN + 1) * 2 +
      2 * slotnr; // TODO use more constants and explain how to calc this
...
int phy_bs_proc_rach(PhyBS phy, int timing_diff) {
  ...
  for (int i = 0; i < nfft; i++) {
    if (common->pilot_sc[i] == OFDMFRAME_SCTYPE_DATA &&
      ...
      modem_demodulate_soft(common->mcs_modem[mcs], phy->rach_buffer[i],
                            &symbol, &demod_buf[written_samps]);
  ...
  if (lchan_verify_crc(chan)) {
    ...
    mac_bs_add_new_ue(phy->mac, rach_userid, rach_try_cnt, phy->fs_rach,
                      timing_diff);
  } else {
...
void phy_bs_rx_symbol(PhyBS phy, float complex *rxbuf_time) {
  ...
  if (sfn == 0 && common->rx_symbol == SUBFRAME_LEN - SLOT_LEN - 2) {
    ...
    if (phy->fs_rach != NULL) {
      ofdmframesync_reset(
          phy->fs_rach); // if we didnt find a new user in last RACH, sync
                         // object still exists. Reset it
    } else {
      phy->fs_rach = ofdmframesync_create(
          nfft, cp_len, 0, phy->common->pilot_sc, _ofdm_rx_rach_cb, phy);
    }
  }
  ...
  if (sfn == 0 && common->rx_symbol >= SUBFRAME_LEN - SLOT_LEN - 2) {
    ...
    if (phy->fs_rach && !ofdmframesync_is_synced(phy->fs_rach)) {
      int offset =
          ofdmframesync_find_data_start(phy->fs_rach, rxbuf_time, rx_sym);
      if (offset != -1) {
        phy->rach_timing =
            offset +
            (nfft + cp_len) * (common->rx_symbol -
        ...
        LOG(INFO,
            ...
            ofdmframesync_get_cfo(phy->fs_rach) * samplerate / 6.28);
        // rach can be unaligned to symbol boundaries. receive rx_sym-offset
        // samps
        ofdmframesync_execute(phy->fs_rach, rxbuf_time + offset,
                              rx_sym - offset);
      }
    } else if (phy->fs_rach) {
      ...
      if (phy->rach_timing <= 0)
        ...
        ofdmframesync_execute(phy->fs_rach, rxbuf_time,
