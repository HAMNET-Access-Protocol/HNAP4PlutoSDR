...
void phy_config_load_file(char *config_file) {
  config_t cfg;
  config_setting_t *phy_settings = NULL, *subcarrier_settings = NULL,
  ...
  if (phy_settings != NULL) {
    ...
    config_setting_lookup_float(phy_settings, "coarse_cfo_filt_param",
                                &coarse_cfo_filt_param);
    config_setting_lookup_float(phy_settings, "agc_rssi_filt_param",
                                &agc_rssi_filt_param);
    config_setting_lookup_int(phy_settings, "agc_change_threshold",
                              &agc_change_threshold);
    config_setting_lookup_int(phy_settings, "agc_desired_rssi",
                              &agc_desired_rssi);
    config_setting_lookup_int(phy_settings, "demo_mode", &demo_mode);
    subcarrier_settings =
        config_setting_get_member(phy_settings, "subcarrier_alloc");
    if (subcarrier_settings != NULL &&
        config_setting_length(subcarrier_settings) > 0) {
      ...
      for (int i = 0; i < nfft; i++) {
        subcarrier_alloc[i] =
            (char)config_setting_get_int_elem(subcarrier_settings, i);
        ...
        default:
          LOG(ERR,
              "[PHY CONFIG] error when parsing SC allocation. %d is an unknown "
              "alloc type\n",
              subcarrier_alloc[i]);
          exit(EXIT_FAILURE);
          break;
    ...
    if (symbol_settings != NULL && config_setting_length(symbol_settings) > 0) {
      ...
      for (int i = 0; i < SLOT_LEN; i++) {
        pilot_symbols[i] =
            (char)config_setting_get_int_elem(symbol_settings, i);
        if (pilot_symbols[i] == PILOT) {
          pilot_symbols_per_slot++;
        } else if (pilot_symbols[i] != NO_PILOT) {
          LOG(ERR,
              "[PHY CONFIG] error when parsing symbol allocation. %d is an "
              "unknown allocation\n",
              pilot_symbols[i]);
        }
      }
