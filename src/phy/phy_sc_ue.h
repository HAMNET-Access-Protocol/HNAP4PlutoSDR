...
struct PhyUE_s {
  ...
  sc_receiver rec; // receiver for singlecarrier phy

  // Variables to store the decoded slot assignments.
  // Set to 1 if the corresponding slot is allocated for this client, 0
  // otherwise
  ...
  struct MacUE_s
      *mac; // Pointer to MAC layer. Needed to call mac interface function
  ...
  float prev_cfo;
  // Flag stores whether prev_cfo already holds an estimate, i.e. if we were
  // synced before
  int has_synced_once;
