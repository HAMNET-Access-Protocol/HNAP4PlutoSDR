...
// is used to properly align UL and DL over the air and compensate for FIR
// delays
...
void *thread_phy_bs_tx(void *arg) {
  ...
  pthread_cond_t *scheduler_signal =
      ((struct tx_th_data_s *)arg)->scheduler_signal;
  ...
  while (1) {
    ...
    for (int symbol = 0; symbol < SUBFRAME_LEN / 2; symbol++) {
      bs->platform_tx_push(bs);
      bs->platform_tx_prep(bs, txbuf_time + buflen - INTER_SYMB_OFFSET, 0,
                           INTER_SYMB_OFFSET);
      TIMECHECK_START(timecheck_bs_tx);
      phy_bs_write_symbol(phy, txbuf_time);
      phy_bs_write_symbol(phy, txbuf_time + 1 * (nfft + cp_len));

      bs->platform_tx_prep(bs, txbuf_time, INTER_SYMB_OFFSET,
                           buflen - INTER_SYMB_OFFSET);
      // run scheduler. TODO tweak signaling time: after ULCTRL is received, but
...
int main(int argc, char *argv[]) {
  ...
  if (pthread_create(&bs_phy_rx_slot_th, NULL, thread_phy_bs_rx_slot,
                     &rx_slot_th_data) != 0) {
  ...
  // start MAC thread
  if (pthread_create(&bs_mac_th, NULL, thread_mac_bs_scheduler, &mac_th_data) !=
      0) {
  ...
  while (1) {
    ...
    for (int userid = 0; userid < MAX_USER; userid++) {
      if (mac->UE[userid] != NULL) {
        ...
        LOG(INFO, "UL mcs %d DL mcs %d\n", mac->UE[userid]->ul_mcs,
            mac->UE[userid]->dl_mcs);
