...
void create_tx_buf(PhyBS phy_bs, uint mcs, float complex* buf_in)
{
    ...
    memcpy(last_payload,payload,1024);
...
void mac_callback(LogicalChannel chan)
{
	...
	uint biterr = count_bit_errors_array(chan->data,last_payload+chan->payload_len*chan->writepos,chan->payload_len);
	total_error+=biterr;
	...
	log_bin((uint8_t*)chan->data,chan->payload_len,"rx.bin","a");
	log_bin((uint8_t*)payload+chan->payload_len*chan->writepos,chan->payload_len,"tx.bin","a");
...
int main()
{
	...
	while (!stop)
	{
		...
		// Adjust for rx_offset
		if (abs(phy_ue->rx_offset)>=2) {
			if (phy_ue->rx_offset < 0) {
				phy_ue->rx_offset += buf_len;
			}
			//rx_samps = phy_ue->rx_offset;
			printf("rxoffset: %d\n",phy_ue->rx_offset);
		}
